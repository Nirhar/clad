// RUN: %cladenzymeclang %s -I%S/../../include -oReverseMode.out | FileCheck %s 
...
// CHECK-NEXT:    clad::Gradient<3> grad = __enzyme_autodiff_myfunction(myfunction, x, y, z);
...
// CHECK-NEXT:     double *d_arr = _d_arr.ptr();
// CHECK-NEXT:     __enzyme_autodiff_summer(summer, arr, d_arr, n);
// CHECK-NEXT: }

double f4(double* arr1, int n,  double*arr2, double m){
    double sum=0;
    for(int i=0;i<n;i++){
        sum+=arr1[i]*arr1[i];
    }
...
// CHECK-NEXT:     __enzyme_autodiff_summer2(summer2, arr1, d_arr1, n, arr2, d_arr2, m);
// CHECK-NEXT: }
...
// CHECK-NEXT:     double *d_arr = _d_arr.ptr();
// CHECK-NEXT:     clad::Gradient<2> grad = __enzyme_autodiff_mixed(mixed, arr, d_arr, x, n, y);
// CHECK-NEXT:     * _d_x = grad.d_arr[0U];
// CHECK-NEXT:     * _d_y = grad.d_arr[1U];
// CHECK-NEXT: }
...
int main(){
  ...
  f_grad.execute(f2_x,f2_y,f2_z,&f2_res[0],&f2_res[1],&f2_res[2]);
  printf("d_x = %.2f, d_y = %.2f, d_z = %.2f\n", f2_res[0], f2_res[1], f2_res[2]); 
  //CHECK-EXEC: d_x = 20 d_y = 15 d_z = 12
  ...
  auto f5_grad=clad::gradient<clad::opts::use_enzyme>(mixed);
