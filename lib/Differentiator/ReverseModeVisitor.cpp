...
namespace clad {
  ...
  void ReverseModeVisitor::DifferentiateWithEnzyme() {
    ...
    for (int i = 0; i < numParams; i++) {
      ...
      // If the Original parameter is not of array/pointer type, then we dont
    ...
    if (enzymeRealParams.size()) {
      ...
      DeclarationName gradIdent = &m_Context.Idents.get("EnzymeGradient");
      LookupResult gradR(m_Sema, gradIdent, noLoc, Sema::LookupUsingDeclName,
      ...
      TT = m_Context.getElaboratedType(
          ETK_None, NS,
          m_Sema.CheckTemplateIdType(TemplateName(gradDecl), noLoc, TLI));
    } else {
      TT = m_Context.VoidTy;
    }
    ...
    QualType enzymeFunctionType =
        m_Sema.BuildFunctionType(TT, enzymeParamsType, noLoc, nameEnzyme,
                                 originalFnType->getExtProtoInfo());
    ...
    if (enzymeRealParams.size() != 0) {
      auto gradDeclStmt = BuildVarDecl(TT, "grad", enzymeCall, true);
      ...
      for (int i = 0; i < enzymeRealParams.size(); i++) {
        ...
        auto ME = m_Sema
                      .ActOnMemberAccessExpr(getCurrentScope(), gradDeclExpr,
                                             noLoc, tok::TokenKind::period, SS,
                                             noLoc, member,
                                             /*ObjCImpDecl=*/nullptr)
                      .getAs<MemberExpr>();
